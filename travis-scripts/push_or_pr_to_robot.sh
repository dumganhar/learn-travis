...
# It's a push or merge, so we push to repo of robot
    ...
    curl --user "${MY_NAME}:${MY_PW}" --request POST --data "{ \"title\": \"test : updating submodule reference to latest autogenerated bindings\", \"body\": \"\", \"head\": \"dumganhar:${TRAVIS_BRANCH}\", \"base\": \"${TRAVIS_BRANCH}\"}" https://api.github.com/repos/CocosRobot2/learn-travis/pulls
fi
